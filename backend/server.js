...
app.post("/booking", (req, res) => {
  const { name, age, address, destination, bookingtime, bus_name, price, travel_date } = req.body;
  const userMobile = req.body.Mobile_number || req.body.mobile;

  if (!name || !age || !address || !userMobile || !travel_date) {
  ...
  db.query(
    ...
    async (err, result) => {
      ...
      try {
        ...
        sendSMS(userMobile, msg).catch(err => console.error("SMS failed to send:", err));
